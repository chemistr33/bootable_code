...
struct filesystem *filesystems[LAMEOS_MAX_FILESYSTEMS];
struct file_descriptor *file_descriptors[LAMEOS_MAX_FILE_DESCRIPTORS];

static struct filesystem **
fs_get_free_filesystem ()
{
  int i = 0;
  for (i = 0; i < LAMEOS_MAX_FILESYSTEMS; i++)
    {
      if (filesystems[i] == 0)
        {
...
{
  int res = -ENOMEM;
  for (int i = 0; i < LAMEOS_MAX_FILE_DESCRIPTORS; i++)
    {
...
{
  if (fd <= 0 || fd >= LAMEOS_MAX_FILE_DESCRIPTORS)
    {
      return 0;
    }
...
fs_resolve (struct disk *disk)
{
  ...
  for (int i = 0; i < LAMEOS_MAX_FILESYSTEMS; i++)
    {
...
FILE_MODE
file_mode_get_by_string (const char *str)
{
...
fopen (const char *filename, const char *mode_str)
{
  ...
  // 0:/ vs. 0:/test.txt? Can't have just a root path...
  if (!root_path->first)
    {
      res = -EINVARG;
      goto out;
  ...
  FILE_MODE mode = file_mode_get_by_string (mode_str);
